...
ui <- fluidPage(
  ...
  sidebarLayout(
    ...
    mainPanel(
      column(6,
             plotOutput("plot_detector1_spectrum", height = 200, width = 300),
             plotOutput("plot_detector2_spectrum", height = 200, width = 300),
             plotOutput("plot_light_spectrum", height = 200, width = 300)
      ),
      column(6,
             plotOutput("plot_detector1_right", height = 200, width = 300),
             plotOutput("plot_detector2_right", height = 200, width = 300),
...
server <- function(input, output, session) {
  ...
  output$reactive_sentence <- renderText({
    ...
    sentence <- paste("If the first detector shows", input_value, unit,", then the true spectrum contains", input_value * ratio1, "µE and the second detector must show", input_value * ratio2, unit," to have the same amount of light.")
    HTML(sentence)
  })
}
